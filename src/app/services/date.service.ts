...
export class DateService {
    ...
    getShortDateDisplay(dateValue: number): string | null{
        let myFormattedDate = pipe.transform(dateValue, 'shortDate');
        return myFormattedDate;
    }

    getMediumDateDisplay(dateValue: number): string | null {
        let myFormattedDate = pipe.transform(dateValue, 'mediumDate');
        return myFormattedDate;
    }

    getShortDateAndTimeDisplay(dateValue: number): string | null {
        let myFormattedDate = pipe.transform(dateValue, 'short');
        return myFormattedDate;
    }

    getShortTimeDisplay(dateValue: number): string | null{
